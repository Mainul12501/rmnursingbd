...
class UpdateNewsEventRequest extends FormRequest
{
    ...
    {
        /** @var \App\Models\NewsEvent|string|int|null $newsEvent */
        $newsEvent = $this->route('news_event');
        $newsEventId = $newsEvent instanceof NewsEvent ? $newsEvent->id : $newsEvent;

        return [
            'news_event_category_id' => ['required', 'integer', 'exists:news_event_categories,id'],
            'title' => ['required', 'string', 'max:255', Rule::unique('news_events', 'title')->ignore($newsEventId)],
            'slug' => ['nullable', 'string', 'max:255', Rule::unique('news_events', 'slug')->ignore($newsEventId)],
